...
def test_config_reload(ingress_controller, configmap, config_dict, collect_coverage):
    ...
    wait_for_watch_streams_ready(ingress_controller)
    ...
    assert_config_change(ingress_controller)
...
def test_no_reload_loop_on_repeated_events(
    ...
    assert_no_reload_loop(ingress_controller, max_reloads=0, timeout=30)

    # Verify operator is still healthy and configuration is unchanged
    final_response = send_socket_command(ingress_controller, "dump all")

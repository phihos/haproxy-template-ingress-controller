...
async def test_dataplane_client_validate_valid_configuration(
    ...
    with progress_context("test_validate_valid_configuration", reporter) as progress:
        progress.phase("API_CLIENT_SETUP", "Creating Dataplane API client")
...
async def test_dataplane_client_deploy_configuration(
    ...
    with progress_context("test_deploy_configuration", reporter) as progress:
        progress.phase("API_CLIENT_SETUP", "Creating production Dataplane API client")
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_dataplane_client_timeout_handling(
    validation_dataplane_client, haproxy_configs
):
    ...
    if not base_url.endswith("/v3"):
        base_url += "/v3"
    auth = ("admin", "adminpass")  # Correct auth from HAProxy config
    client = DataplaneClient(base_url, timeout=0.001, auth=auth)  # Very short timeout

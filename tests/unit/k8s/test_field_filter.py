...
def test_field_path_with_special_chars():
...
def test_array_element_removal():
...
def test_wildcard_field_removal():
...
def test_validate_ignore_fields():
...
def test_invalid_field_path_in_remove(monkeypatch):
    ...
    with mock_module_attributes(field_filter_module, logger=mock_logger):
...
def test_field_path_too_long_in_remove(monkeypatch):
...
def test_unexpected_error_in_remove(monkeypatch):
    ...
    with mock_module_attributes(
        field_filter_module,
        _compile_jsonpath_filter=mock_compile,
        logger=mock_logger,
    ):
...
def test_list_navigation_errors():
...
def test_final_part_list_errors():
...
def test_validate_ignore_fields_with_invalid(monkeypatch):
    ...
    assert mock_logger.warning.call_count >= 3  # Empty, None, too long, invalid syntax


def test_from_kopf_index_with_field_filtering():
...
def test_from_kopf_index_without_field_filtering():

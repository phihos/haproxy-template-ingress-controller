...
class GlobalBaseHardenRejectPrivilegedPorts(BaseModel):
    """
    GlobalBaseHardenRejectPrivilegedPorts
    """ # noqa: E501
    quic: Optional[StrictStr] = None
    tcp: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["quic", "tcp"]

    @field_validator('quic')
    def quic_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    @field_validator('tcp')
    def tcp_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

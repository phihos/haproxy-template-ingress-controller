...
from .types import DataplaneAPIError, MapChange, ReloadInfo, RuntimeOperationResult
            ...
            with metrics.time_dataplane_api_operation(f"map_{operation.operation}"):
                try:
                    if operation.operation == "add":
                        ...
                        response = await add_map_entry.asyncio_detailed(
                            client=client,
                            name=map_name,
                            body=body,
                        )
                        reload_info = ReloadInfo.from_response(response, self.endpoint)
                        reload_infos.append(reload_info)
                        await logger.adebug(
                        ...
                        response = await replace_runtime_map_entry.asyncio_detailed(
                            client=client,
                            name=map_name,
                            id=operation.key,
                            body=body,
                        ...
                        response = await delete_runtime_map_entry.asyncio_detailed(
            ...
            with metrics.time_dataplane_api_operation(f"acl_{operation.operation}"):
                try:
                    if operation.operation == "add":
                        ...
                        response = await post_services_haproxy_runtime_acls_parent_name_entries.asyncio_detailed(
                            client=client,
                            parent_name=acl_id,
                            body=body,
                        )
                        ...
                        response = await delete_services_haproxy_runtime_acls_parent_name_entries_id.asyncio_detailed(
                            client=client,
                            parent_name=acl_id,
                            id=operation.key,
                        )
    ...
    async def update_server_state(
        ...
        with metrics.time_dataplane_api_operation("server_state_update"):
            try:
                ...
                response = await replace_runtime_server.asyncio_detailed(
                    client=client,
